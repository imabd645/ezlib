...
struct Conn {
    ...
    //
    // The body reaches EZ through the FFI callback as a `const char*`, which the
    // interpreter turns back into a string with std::string(const char*) -- so it
    // stops dead at the first NUL. Every binary upload was silently truncated
    // there: a PNG carries NULs in its IHDR length field at byte 8, so a PNG
    // POST arrived 8 bytes long, and the documented req["files"] feature could
    // not work for any binary format.
    //
    // These let EZ pull the bytes out by pointer+length instead. They point into
    // client_handler's local `body`, which is alive for exactly as long as the
    // callback runs (the connection thread blocks inside it), and the whole Conn
    // is erased from g_active_sockets the moment the callback returns -- so a
    // stale req_id can never dereference a dead frame.
...
void client_handler(SOCKET client_socket, std::string client_ip) {
  ...
  next_request:
...

    {
        ...
        // Safe to hold a pointer into `body`: the callback below runs to
        // completion before this frame returns, and the entry is erased
        // immediately afterwards.
        c.body_ptr = body.data();
        c.body_len = (long long)body.size();
...
extern "C" {
    ...
    // Send a byte RANGE of a file (for a 206 Partial Content reply).
    //
    // Without this, a media element could not seek and an interrupted download
    // could not resume: the server advertised no ranges and always sent the
    // whole file from byte zero, so a browser scrubbing a video re-fetched it
    // from the start every time.
    //
    // `header_str` is the complete header block (the caller writes the 206
    // status line, Content-Range and Content-Length). offset/length address the
    // slice; length <= 0 means "to end of file".
    __declspec(dllexport) int http_server_send_file_range(int client_id, const char* header_str,
        ...
        char buf[64 * 1024];
        long long remaining = (length > 0) ? length : -1;
        while (remaining != 0) {
            size_t want = sizeof(buf);
            if (remaining > 0 && remaining < (long long)want) want = (size_t)remaining;
    ...
    // ── Chunked streaming ───────────────────────────────────────────────────
    //
    // Every response used to be built as one string with a Content-Length, so
    // the whole body had to exist in memory before a single byte went out. That
    // ruled out server-sent events, progress feeds, and any export larger than
    // RAM.
    //
    // These three send a Transfer-Encoding: chunked response incrementally. They
    // are safe to call from the EZ handler because the handler runs on the
    // interpreter's event-loop thread while the connection thread is parked
    // inside the FFI callback -- only one thread touches this socket at a time.
    //
    //   stream_begin(id, headers)  -> status line + headers (caller includes
    //                                 "Transfer-Encoding: chunked" and the
    //                                 terminating blank line)
    //   stream_write(id, buf, n)   -> one chunk
    //   stream_end(id)             -> terminating zero-length chunk
    __declspec(dllexport) int http_server_stream_begin(int client_id, const char* header_str) {
        ...
        if (header_str && !send_all(s, header_str, strlen(header_str))) return -1;
        return 1;
    }

    __declspec(dllexport) int http_server_stream_write(int client_id, const char* data, int len) {
        SOCKET s = INVALID_SOCKET;
        {
            std::lock_guard<std::mutex> lock(g_sockets_mutex);
            auto it = g_active_sockets.find(client_id);
        ...
        if (s == INVALID_SOCKET || len <= 0) return 0;

        // chunk = <hex length> CRLF <data> CRLF
        char hdr[32];
        int hn = snprintf(hdr, sizeof(hdr), "%X\r\n", (unsigned)len);
        if (!send_all(s, hdr, hn)) return -1;
        if (!send_all(s, data, (size_t)len)) return -1;
        if (!send_all(s, "\r\n", 2)) return -1;
        return 1;
    }

    __declspec(dllexport) int http_server_stream_end(int client_id) {
        SOCKET s = INVALID_SOCKET;
        bool keep_alive = false;
        {
            std::lock_guard<std::mutex> lock(g_sockets_mutex);
            ...
            s = it->second.sock;
            keep_alive = it->second.keep_alive;
            // Chunked framing tells the client where the body ends, so the
            // connection can legitimately be reused.
            it->second.close_after = !keep_alive;
        }
        if (s == INVALID_SOCKET) return -1;
        if (!send_all(s, "0\r\n\r\n", 5)) return -1;
        return 1;
    ...
    // Raw request body, by pointer and length, for the request currently being
    // handled. Valid ONLY while the EZ callback for `client_id` is running.
    //
    // EZ side:
    //   ptr = os_call(bodyPtr, "ptr", req["id"])
    //   n   = os_call(bodyLen, "int", req["id"])
    //   raw = buf_to_str(os_buffer_from_ptr(ptr, n))
    //
    // os_buffer_from_ptr copies exactly n bytes and buf_to_str builds the string
    // from iterators, so NULs survive the whole way -- unlike the const char*
    // the callback itself receives.
    __declspec(dllexport) long long http_request_body_ptr(int client_id) {
        std::lock_guard<std::mutex> lock(g_sockets_mutex);
        auto it = g_active_sockets.find(client_id);
        if (it == g_active_sockets.end()) return 0;
        return (long long)(uintptr_t)it->second.body_ptr;
    }

    __declspec(dllexport) long long http_request_body_len(int client_id) {
        std::lock_guard<std::mutex> lock(g_sockets_mutex);
        auto it = g_active_sockets.find(client_id);
        if (it == g_active_sockets.end()) return 0;
        return it->second.body_len;
    }
