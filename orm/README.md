...
| Declaration | Meaning | `foreignKey` names |
|---|---|---|
| `hasMany("Post", "userId")` | one-to-many | the column on the **target** table pointing back here |
| `belongsTo("User", "userId")` | many-to-one | the column on **this** table pointing at the target |
...
- **Relations are one-to-many and many-to-one only.** `hasMany`/`belongsTo` cover the
  ...
  defeat the purpose.
- **Relation loading requires extending `Model`.** Lazy access and change tracking
  ...
  you `add()` it.
- **The identity map is per-session and never evicts.** Every row a session loads
...
A primary key your code sets — a username, an email, a UUID — works the same
as a database-generated one:
...
> **Fixed in 1.1.0.** Before that, the session decided between INSERT and
> UPDATE by asking whether the primary key held a value. That is only a
> reliable signal for keys the database generates: an application-assigned key
> is set *before* the row exists, so a brand new object was flushed as an
> UPDATE, matched no rows, raised nothing, and saved nothing.
>
> The session now tracks whether an instance actually came from the database.
> Objects returned by a query are updates; everything else is an insert. Two
> consequences worth knowing:
>
> - Adding an object whose key already exists now raises a constraint error
>   instead of silently overwriting the row. To change an existing row, load
>   it first.
> - An object you insert joins the identity map, so a later query in the same
>   session returns that same object rather than a second copy of the row.
>   Changing a field after the insert is picked up by the next `commit()`.
