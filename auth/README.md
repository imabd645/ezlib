# auth — Authentication for the EZ web framework
...
`auth` does what `flask-login` does — track who is logged in and gate routes on it —
...
# Required: teach auth how to turn a stored id back into a user.
...
Or as a decorator, stacked under the route:

```ez
@app.get("/dash")
@auth.login_required
task dash(req) {
    give "members only for " + auth.current_user(req)["name"]
}
```

Decorators apply **bottom-up**: `login_required` wraps `dash`, then `app.get` registers
the wrapped function. Order matters — the guard must sit *below* the route registration,
never above it, or the raw handler is what gets registered.
...
A cookie the client can read but cannot forge — the value travels in the clear with an
HMAC-SHA256 appended, as `base64url(value).hmac`.

```ez
@app.get("/setpref")
task setpref(req) {
    r = { "status": 200, "body": "saved" }
    set_signed_cookie(r, "prefs", "dark-mode", SECRET)
    give r
}

@app.get("/getpref")
task getpref(req) {
    v = get_signed_cookie(req, "prefs", SECRET)     # nil if absent or tampered
    give { "status": 200, "body": str(v) }
}
```

`set_signed_cookie(res, name, value, secret, opts = nil)` takes the same `opts` as
`set_cookie` (`maxAge`, `path`, `secure`, `sameSite`, …).

This is **signing, not encryption** — anyone can read the value, they just cannot change
it. Never put a secret in one. Verification is constant-time, and a tampered cookie
returns `nil` rather than throwing, so a forgery can never be caught and mistaken for a
success path.

These live in auth rather than web because the HMAC does: auth already depends on
web, so the reverse would be circular.
